...
<html>
	...
	<body>
			...
			<% Integer cust = customer.getCustomerId();%>
			
			<% ArrayList<BookingDetail> bookingDetails = new ArrayList<BookingDetail>();
			 bookingDetails = MySQLDataStoreUtilities.getBookingDetails(cust);
			...
			<%  if(!bookingDetails.isEmpty())
				{	
						 ...
						 {
							 Integer customerId1 = bookingDetail.getCustomerId();
							 if(customers.containsKey(customerId1))
							 {
			%>
								...
								<td><label for="fname"><%=customers.get(customerId1).getEmailId()%></label></td>
				...
				%>
								...
								<form action='ViewReservationCustomer.jsp' method='get'>
								...
								<form action='UpdateReservationCustomer.jsp?action=delete' method='get'>
								<td><input TYPE='hidden' name='bookingId' value='<%=bookingDetail.getBookingId()%>'></input>
				...
				%>
						...
						<h5 align='center'>No Reservations Available !</h5>
				<%
						}
				%>
						<!--
